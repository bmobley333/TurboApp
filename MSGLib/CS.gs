...
function fCSCreateMenu() {
  ...
  .createMenu('*** Game')
    ...
    .addSubMenu(SpreadsheetApp.getUi().createMenu('Jump to')
    ...
    .addSeparator()
    .addSubMenu(SpreadsheetApp.getUi().createMenu('Healing')
      .addItem('Natural Healing', 'fCSMenuNaturalHealing')
      .addItem('All Wounds', 'fCSMenuHealAll')
    )
    .addSubMenu(SpreadsheetApp.getUi().createMenu('Rest or Time')
    ...
    .addSubMenu(SpreadsheetApp.getUi().createMenu('Set to Max')
  ...
  .createMenu('Gear')
    ...
    .addSubMenu(SpreadsheetApp.getUi().createMenu('Specific Treasure')
  ...
  .createMenu('RaceClass')
    ...
    .addItem(`Generate Race Gender to Senses`, 'fCSMenuGenerateRaceGenderToVision')
    .addSeparator()
...
function fCSMenuRoll() {fCSRunMenuOrButton('Roll');}
function fCSMenuRollLucked() {fCSRunMenuOrButton('RollLucked');}
function fCSMenuRollFree() {fCSRunMenuOrButton('RollFree');}
function fCSMenuShowSidebar() {fCSRunMenuOrButton('ShowSidebar');}
function fCSMenuScrollToNish() {fCSRunMenuOrButton('ScrollToNish');}
function fCSMenuScrollToGear() {fCSRunMenuOrButton('ScrollToGear');}
function fCSMenuNishStartOrEnd() {fCSRunMenuOrButton('NishStartOrEnd');}
function fCSMenuLucktoFull() {fCSRunMenuOrButton('LuckToFull');}
function fCSMenuMetaToFull() {fCSRunMenuOrButton('MetaToFull');}
function fCSMenuMetaFlood() {fCSRunMenuOrButton('MetaFlood');}
function fCSMenuRefreshAbilities() {fCSRunMenuOrButton('RefreshAbilities');}
function fCSMenuLoadMonsters() {fCSRunMenuOrButton('LoadMonsters');}
function fCSMenuGMAward() {fCSRunMenuOrButton('GMAward');}
function fCSMenuRollRndTreasure() {fCSRunMenuOrButton('RollRndTreasure');}
function fCSMenuChaosCrystal() {fCSRunMenuOrButton('ChaosCrystal');}
function fCSMenuRollArtifact() {fCSRunMenuOrButton('RollArtifact');}
function fCSmenuRollGear() {fCSRunMenuOrButton('RollGear');}
function fCSMenuRollSockets() {fCSRunMenuOrButton('RollSockets');}
function fCSMenuRollSocketGem() {fCSRunMenuOrButton('RollSocketGem');}
function fCSMenuRollValuables() {fCSRunMenuOrButton('RollValuables');}
function fCSMenuNaturalHealing() {fCSRunMenuOrButton('NaturalHealing');}
function fCSMenuHealAll() {fCSRunMenuOrButton('HealAll');}
function fCSMenuClearRollLog() {fCSRunMenuOrButton('ClearRollLog');}
function fCSMenuRest() {fCSRunMenuOrButton('Rest');}
function fCSMenuSleep() {fCSRunMenuOrButton('Sleep');}
function fCSMenuNewGameSession() {fCSRunMenuOrButton('NewGameSession');}
function fCSMenuRestAllToMax() {fCSRunMenuOrButton('RestAllToMax');}
function fCSMenuCalcVitMax() {fCSRunMenuOrButton('CalcVitMax');}
function fCSMenuClearChanneledMeta() {fCSRunMenuOrButton('ClearChanneledMeta');}
function fCSMenuClearSk1Sk2CheckBoxes() {fCSRunMenuOrButton('ClearSk1Sk2CheckBoxes');}
function fCSMenuClearMonsters() {fCSRunMenuOrButton('ClearMonsters');}
function fCSMenuClearMorphs() {fCSRunMenuOrButton('ClearMorphs');}
function fCSMenuClearNish() {fCSRunMenuOrButton('ClearNish');}
// Gear
function fCSMenuShowGear() {fCSRunMenuOrButton('ShowGear');}
function fCSMenuHideGear() {fCSRunMenuOrButton('HideGear');}
// RaceClass Menu
function fCSMenuRefreshRaceClass() {fCSRunMenuOrButton('RefreshRaceClass');}
function fCSMenuBuildRaceDropDown() {fCSRunMenuOrButton('BuildRaceDropDown');}
...
function fCSMenuCreateKL() {fCSRunMenuOrButton('CreateKL');}
function fCSMenuRefreshAll() {fCSRunMenuOrButton('RefreshMenu');}
// Designer Menu
function fCSMenuHideAll() {fCSRunMenuOrButton('HideAll');}
function fCSMenuUn_HideAll() {fCSRunMenuOrButton('Un_HideAll');}
// End Menu Functions


// Image Button Functions //////////////////////////////////////////////////////////////////////////////////////////////////
function fCSButtond20() {fCSRunMenuOrButton('Roll');}
function fCSButtonRefreshAbilities() {fCSRunMenuOrButton('RefreshAbilities');}
function fCSButtonScrollToNish() {fCSRunMenuOrButton('ScrollToNish');}
...
function fCSRunMenuOrButton(menuChoice) { 
  try {
    ...
    switch (menuChoice) {
      ...
      case 'LuckToFull': fCSLuckToMax(); break;
      case 'MetaToFull': fCSMetaToFull(); break;
      case 'MetaFlood': fCSMetaFlood(); break;
      case 'RefreshAbilities': fCSRefreshAbilities(); break;
      case 'LoadMonsters': fCSLoadMonsters(); break;
      case 'GMAward': fCSGMAward(); break;
      case 'RollRndTreasure': fCSRollRndTreasure(); break;
      case 'ChaosCrystal': fCSChaosCrystal(); break;
      case 'RollArtifact': fCSRollArtifact(); break;
      case 'RollGear': fCSRollGear(); break;
      case 'RollSockets': fCSRollSockets(); break;
      case 'RollSocketGem': fCSRollSocketGem(); break;
      case 'RollValuables': fCSRollValuables(); break;
      case 'NaturalHealing': fCSNaturalHealing(); break;
      case 'HealAll': fCSHealAll(); break;
      case 'ClearRollLog': fCSClearRollLog(); break;
      case 'Rest': fCSRest(); break;
      case 'Sleep': fCSSleep(); break;
      case 'NewGameSession': fCSNewGameSession(); break;
      case 'RestAllToMax': fCSRestAllToMax(); break;
      case 'CalcVitMax': fCSCalcVitMax(); break;
      case 'ClearChanneledMeta': fCSMetaChnlClear(); break;
      case 'ClearSk1Sk2CheckBoxes': fCSClearSk1Sk2CheckBoxes(); break;
      case 'ClearMonsters': fCSClearMonsters(); break;
      case 'ClearMorphs': fCSClearMorphs(); break;
      case 'ClearNish': fCSClearNish(); break;
      // Gear
      case 'ShowGear': fCSShowGear(true); break;
      case 'HideGear': fCSShowGear(false); break;
      // RaceClass Menu
      case 'RefreshRaceClass': fCSRefreshRaceClass(); break;
      case 'BuildRaceDropDown': fCSBuildRaceDropDown(); break;
      ...
      case 'CreateKL': fCSCreateKL(); break;
      case 'RefreshMenu': fCSCreateMenu(); break;
      // Designer Menu
      case 'HideAll': gHideAll('mycs'); break;
      case 'Un_HideAll': gUn_HideAll('mycs'); break;
    }
  } catch (error) {
...
function fCSMetaToFull(updateRollLog = true) {
  ...
  gSaveArraySectionToSheet(gameRef, gameArr, meta_R, meta_R, metaRed_C, metaBlue_C);
...
function fCSMetaFlood(returnResult = false) {
  ...
  for (let i = 0; i < maxMeta.length; i++) {
    gameArr[meta_R][metaRed_C + i] = Math.min(gameArr[meta_R][metaRed_C + i], maxMeta[i]);
  }
...
function fCSMetaChnlClear() {
  ...
  gFillArraySection(game.arr,game.metaChnl_R,game.metaChnl_R,game.metaR_C,game.metaB_C,'');

  // Save the updated chnl section of gameArr back to the sheet
  gSaveArraySectionToSheet(game.ref, game.arr,game.metaChnl_R,game.metaChnl_R,game.metaR_C,game.metaB_C);
...
function fCSNishStartOrEnd() {
  ...
  fCSResetAbilOn();
  fCSClearRollLog();
  fCSPostMyRoll(`${g.bluBS}Nish Starts or Ends:${g.endS}<br>${result}<br>----------------------------------------`)
...
function fCSResetAbilOn() {
  ...
  gFillArraySection(game.arr, game.NishAtr_R, game.abilTblEnd_R, game.on_C, game.on_C, '.');
  gFillArraySection(game.arr, game.gearTblStart_R, game.gearTblEnd_R, game.on_C, game.on_C, '.');
  gSaveArraySectionToSheet(game.ref, game.arr, game.NishAtr_R, game.gearTblEnd_R, game.on_C, game.on_C);
...
function fCSCalcMR(objGame, encTotSum) {
  ...
  const plusMR = parseInt(objGame.arr[objGame.gearPlusMR_R][objGame.gearPlusMR_C], 10) || 0;
  const plusCarry = parseInt(objGame.arr[objGame.gearPlusCarry_R][objGame.gearPlusCarry_C], 10) || 0;
  ...
  for (let c = objGame.gearMRCol1_C, tableCol = 0; c <= objGame.gearMRCol6_C; c += 2, tableCol++) {
    objGame.arr[objGame.gearMRTbl_R][c] = Math.round((6 + atrSpe / 5 + plusMR) * mrMultArr[tableCol]);
    objGame.arr[objGame.gearCarryTbl_R][c] = Math.round((50 + atrStr + plusCarry) * carryMultArr[tableCol]);
  ...
  if (finalMR === 0) SpreadsheetApp.getUi().alert('You are over encumbered, MR is 0.');
...
function fCSCalcMaxSocketedItems(objGame) {
  ...
  const maxSocketItems = Math.min(12, 2 * Math.round(3.2 * Math.log10(charLevel + 1)));

  // Set Socket Item table's rows to white or light grey
  const totalCols = objGame.last_C - objGame.socketedGear_C + 1;
  if (maxSocketItems > 0) {
    objGame.ref.getRange(objGame.socketTblStart_R + 1, objGame.socketedGear_C + 1, maxSocketItems, totalCols)
      .setBackground('white');
  }
  if (maxSocketItems < 12) {
    objGame.ref.getRange(objGame.socketTblStart_R + maxSocketItems + 1, objGame.socketedGear_C + 1, 12 - maxSocketItems, totalCols)
...
function fCSLoadMonsters() {
  ...
  gFillArraySection(game.arr, game.monsterFirst_R, game.monsterLast_R, game.numOfMonsters_C, game.monsterSize_C, '');
...
while (cs_R <= game.monsterLast_R) {
  if (!allMonstersFound && db_R <= gmScreen.monLast_R) {
    if (gmScreen.arr[db_R][gmScreen.monActiveTF_C]) {
      ...
      game.arr[cs_R][game.monsterName_C] = gmScreen.arr[db_R][gmScreen.monCustName_C] ? gSimpleName(gmScreen.arr[db_R][gmScreen.monCustName_C]) : gSimpleName(gmScreen.arr[db_R][gmScreen.monName_C]);

      // Copy columns from Vit to Pic
      for (let i = 0; i <= (game.monsterPic_C - game.monsterVit_C); i++) {
        game.arr[cs_R][game.monsterVit_C + i] = gmScreen.arr[db_R][gmScreen.monVit_C + i];
      }

      game.arr[cs_R][game.monsterSize_C] = fCSExtractMonSize(gmScreen.arr[db_R][gmScreen.monNotes_C]);
...
if (cs_R > game.monsterLast_R && !allMonstersFound) {
  SpreadsheetApp.getUi().alert(`Tell the GM that there are more than ${game.monsterLast_R - game.monsterFirst_R+1} Active Monsters. And the rest are not shown.`);
}

  gSaveArraySectionToSheet(game.ref,game.arr, game.monsterFirst_R, game.monsterLast_R, game.numOfMonsters_C, game.monsterSize_C);
} // End fCSLoadMonsters
...
function fCSExtractMonSize(notesText) {
  ...
  if (notesText.includes('\nSIZE:')) {
    ...
    monSize = (endIndex !== -1) ? notesText.substring(startIndex, endIndex) : notesText.substring(startIndex);
  } else {
    return notesText;
  }
...
function fCSGMAward() {
  ...
  const randomRow = Math.floor(Math.random() * (dataLast_R - dataFirst_R + 1)) + dataFirst_R;
...
function fCSClearMonsters() {
  ...
  // Clear Monster Table
  gFillArraySection(game.arr,game.custMon_R,game.monsterLast_R,game.numOfMonsters_C,game.monsterSize_C,'');

  // Save Monsters to <Game>
  gSaveArraySectionToSheet(game.ref,game.arr,game.custMon_R,game.monsterLast_R,game.numOfMonsters_C,game.monsterSize_C);
...
function fCSClearMorphs() {
  ...
  // Clear Morph Tables
  gFillArraySection(game.arr,game.dataFirst_R,game.abilTblEnd_R,game.morph1_C,game.morph1_C,',');
  gFillArraySection(game.arr,game.dataFirst_R,game.abilTblEnd_R,game.morph2_C,game.morph2_C,',');
  gFillArraySection(game.arr,game.gearTblStart_R,game.gearTblEnd_R,game.morph1_C,game.morph1_C,',');
  gFillArraySection(game.arr,game.gearTblStart_R,game.gearTblEnd_R,game.morph2_C,game.morph2_C,',');

  // Save Monsters to <Game>
  gSaveArraySectionToSheet(game.ref,game.arr,game.dataFirst_R,game.gearTblEnd_R,game.morph1_C,game.morph1_C);
  gSaveArraySectionToSheet(game.ref,game.arr,game.dataFirst_R,game.gearTblEnd_R,game.morph2_C,game.morph2_C);
} // End fCSClearMorphs 
...
function fCSClearSk1Sk2CheckBoxes() {
  ...
  gFillArraySection(gameObj.arr,gameObj.nishStart_R,gameObj.abilTblEnd_R,gameObj.sk1ChkBox_C,gameObj.sk1ChkBox_C,false);
  gFillArraySection(gameObj.arr,gameObj.nishStart_R,gameObj.abilTblEnd_R,gameObj.sk2ChkBox_C,gameObj.sk2ChkBox_C,false);
  gFillArraySection(gameObj.arr,gameObj.gearTblStart_R,gameObj.gearTblEnd_R,gameObj.sk1ChkBox_C,gameObj.sk1ChkBox_C,false);
  gFillArraySection(gameObj.arr,gameObj.gearTblStart_R,gameObj.gearTblEnd_R,gameObj.sk2ChkBox_C,gameObj.sk2ChkBox_C,false);

  // Save Sk1 and Sk2 Check Boxes to <Game>
  gSaveArraySectionToSheet(gameObj.ref,gameObj.arr,gameObj.nishStart_R,gameObj.gearTblEnd_R,gameObj.sk1ChkBox_C,gameObj.sk1ChkBox_C);
  gSaveArraySectionToSheet(gameObj.ref,gameObj.arr,gameObj.nishStart_R,gameObj.gearTblEnd_R,gameObj.sk2ChkBox_C,gameObj.sk2ChkBox_C);
...
function fCSRestAllToMax() {
  ...
  const promptReply = ui.alert('This will reset', 
    'Nish to blank\n' +
    'Meta to Full\n' +
  ...
  if (promptReply !== ui.Button.YES) throw new Error('Operation canceled by user.');
...
function fCSActToMax() {
  ...
  if (!Number.isInteger(actPlus) || actPlus < 0) throw new Error(`"Act +" must be 0 or a positive integer, but is currently "${actPlus}"`);
...
function fCSHealAll() {
  ...
  gSaveArraySectionToSheet(gameRef,gameArr,vitTbl_R,vitTbl_R,vit1st_C,vit3rd_C);
  getObjCSGame(true); 
...
function fCSNaturalHealing(updateRollLog = true) {
  ...
  let heal1st = Math.min(vit1stWnds, Math.round((fCSd(vitMax) + fCSd(vitMax))/4)); 
  let heal2nd = Math.min(vit2ndWnds, Math.round((fCSd(vitMax) + fCSd(vitMax))/8));
  let heal3rd = Math.min(vit3rdWnds, Math.round((fCSd(vitMax) + fCSd(vitMax))/16));
  ...
  if (updateRollLog && vit1stWnds+vit2ndWnds+vit3rdWnds > 0) {
    fCSPostMyRoll(`${g.bluBS}Natural Healing:${g.endS}<br>${heal1st} -> 1st Deg<br>${heal2nd} -> 2nd Deg<br>${heal3rd} -> 3rd Deg<br>`);
  }
...
function fCSLuckToMax(updateRollLog = true) {
  ...
  if (!Number.isInteger(luckPlus) || luckPlus < 0) throw new Error(`"Luck +" must be 0 or a positive integer, but is currently "${luckPlus}"`);
  ...
  if (updateRollLog) fCSPostMyRoll(`${g.bluBS}Luck to Max:${g.endS} ${finalLuckBoxes}<br>`);
...
function fCSRollRndTreasure() {
  ...
  switch (true) {
    case roll <= 2: fCSRollValuables(rnd); break;
    case roll <= 4: fCSRollGear(rnd); break;
    case roll <= 6: fCSChaosCrystal(rnd); break;
    case roll <= 8: fCSRollSocketGem(rnd); break;
    case roll <= 10: fCSRollArtifact(rnd); break;
    default: fCSRollSockets(rnd);
  }
...
function fCSChaosCrystal(rnd = false) {
  ...
  fCSPostMyRoll(`${(rnd) ? 'Loot:<br>' : '--- SPECIFIC TREASURE ---<br>'}${g.bluBS}Chaos Crystal${g.endS} - ${(chaosCrystalUses === 1) ? 'Use' : 'Uses'}(${chaosCrystalUses}): <br>${chaosCrystalAbility}<br>`);
} // End fCSChaosCrystal
...
function fCSRollArtifact(rnd = false) {
  ...
  let randomRow;
  do {
    randomRow = Math.floor(Math.random() * (artDataLast_R - artDataFirst_R + 1)) + artDataFirst_R;
    loopCount++;
  } while (gearArr[randomRow][apCost_C] > Math.max(5, charLvl) && loopCount < 10);

  const artName = gearArr[randomRow][artName_C];
  const apCost = gearArr[randomRow][apCost_C];

  fCSPostMyRoll(`${rnd ? 'Loot:<br>' : '--- SPECIFIC TREASURE ---<br>'}${g.bluBS}${artName}${g.endS} (${apCost}AP)<br>`);
} // End fCSRollArtifact
...
function fCSRollGear(rnd = false) {
  ...
  // Set CS Data
  const charLevel = gCharLvl();
  const maxCr = charLevel <= 4 ? 100 : charLevel <= 9 ? 250 : charLevel <= 20 ? 1000 : 10000;
  ...
  // Pick a random result
  const randomRow = Math.floor(Math.random() * (itemDataLast_R - itemDataFirst_R + 1)) + itemDataFirst_R;
  ...
  const condition = conditionArr[Math.floor(Math.random() * conditionArr.length)];

  fCSPostMyRoll(`${(rnd) ? 'Loot:<br>' : '--- SPECIFIC TREASURE ---<br>'}${g.bluBS}${itemName}${g.endS}<br>Condition: ${condition}<br>`);
} // End fCSRollGear
...
function fCSRollSockets(rnd = false) {
  ...
  const maxGemColor = charLevel <= 4 ? 2 : charLevel <= 9 ? 3 : charLevel <= 20 ? 4 : 5;
  ...
  const itemRolled = fCSd(3) === 1 ? weaponArr[Math.floor(Math.random() * weaponArr.length)] : itemArr[Math.floor(Math.random() * itemArr.length)];

  // Convert slotColorArr to a comma-separated string
  const slotColorsText = slotColorArr.join(', ');

  // Post the result
  fCSPostMyRoll(`${(rnd) ? 'Loot:<br>' : '--- SPECIFIC TREASURE ---<br>'}${g.bluBS}Socketed Item${g.endS}:<br>${itemRolled} with ${slotColorsText} ${numGemSlots === 1 ? 'slot' : 'slots'}<br>`);
...
function fCSRollSocketGem(rnd = false) {
  ...
  // Determine maxGemColor based on charLevel (which is socketColors index + 1)
  let maxGemColor = charLevel <= 4 ? 2 : charLevel <= 9 ? 3 : charLevel <= 20 ? 4 : 5;
  ...
  while (!gemFound) {
    let r = Math.floor(Math.random() * (dataLast_R - dataFirst_R + 1)) + dataFirst_R;
  ...
  fCSPostMyRoll(`${(rnd) ? 'Loot:<br>' : '--- SPECIFIC TREASURE ---<br>'}${g.bluBS}Socket Gem:${g.endS}<br>${critArr[gemFound_R][gemColor_C]} Gem:<br>${critArr[gemFound_R][gemDesc_C]}<br>`);
...
function fCSRollValuables(rnd = false) {
  ...
  let maxTreasureColor = charLevel <= 4 ? 2 : charLevel <= 9 ? 3 : charLevel <= 20 ? 4 : 5;
  maxTreasureColor = Math.min(fCSd(maxTreasureColor) - 1, fCSd(maxTreasureColor) - 1);
  ...
  while (!treasureFound) {
    let r = Math.floor(Math.random() * (dataLast_R - dataFirst_R + 1)) + dataFirst_R;
    if (treasureColorArr.indexOf(critArr[r][treasureColor_C]) <= maxTreasureColor) {
      treasureFound = true;
      treasureFound_R = r;
    }
  }

  fCSPostMyRoll(`${(rnd) ? 'Loot:<br>' : '--- SPECIFIC TREASURE ---<br>'}${g.bluBS}Treasure:${g.endS}<br>${critArr[treasureFound_R][treasureDesc_C]}<br>`);
...
function fCSBuildRaceDropDown() {
    ...
    gSaveArraySectionToSheet(csRCRef, csRCArr, csRCRace_R, csRCRace_R, csRCNotes_C, csRCPic_C);
...
function fCSUpdateRaceNotesPic() {
  ...
  if (!raceID) throw new Error(`Choose a new race from the dropdown list. I cannot find the race "${myRaceNameID}"`);
...
function fCSGenerateRaceGenderToVision() {
  ...
  csRCArr[csRCGender_R][csRCVal_C] = fCSDDmaker(csRCGender_R,csRCVal_C,dbRaceArr,dbRace_R,dbGenderChoices_C);
  csRCArr[csRCHair_R][csRCVal_C] = fCSDDmaker(csRCHair_R,csRCVal_C, dbRaceArr,dbRace_R,dbHairChoices_C, dbHairSuffix_C);
  csRCArr[csRCEyes_R][csRCVal_C] = fCSDDmaker(csRCEyes_R,csRCVal_C,dbRaceArr,dbRace_R,dbEyeChoices_C);
  csRCArr[csRCAge_R][csRCVal_C] = fCSStatRoller(dbRaceArr,dbRace_R,dbAgeRoll_C,dbAgeMaxRoll_C,dbAgeStatic_C,dbAgeSuffix_C);
  csRCArr[csRCOldAge_R][csRCVal_C] = dbRaceArr[dbRace_R][dbOldAgeCheck_C];
  csRCArr[csRCHeight_R][csRCVal_C] = fCSStatRoller(dbRaceArr,dbRace_R,dbHeightRoll_C,dbHeightMaxRoll_C,dbHeightStatic_C,dbHeightSuffix_C);
  csRCArr[csRCWeight_R][csRCVal_C] = fCSStatRoller(dbRaceArr,dbRace_R,dbWeightRoll_C,dbWeightMaxRoll_C,dbWeightStatic_C,dbWeightSuffix_C);
  csRCArr[csRCOther_R][csRCVal_C] = fCSStatRoller(dbRaceArr,dbRace_R,dbOtherRoll_C,dbOtherMaxRoll_C,'', dbOtherSuffix_C);
  ...
  gSaveArraySectionToSheet(csRCRef, csRCArr, csRCGender_R, csRCVision_R, csRCVal_C, csRCVal_C);
...
function fCSStatRoller(dbArr, db_R, dbRoll_C, dbMax_C, dbStatic_C = '', dbSuffix_C = '') {
    ...
    if (!rollResult && dbArr[db_R][dbRoll_C]) {
        ...
        if (!rollMatch) {
            throw new Error(`In fCSStatRoller, the incorrect formula "${rollStr}" was passed.`);
        }
...
function fCSConvertFtIn(inches) {
    // Validate input
    if (isNaN(inches) || inches < 0) throw new Error(`Error in fCSConvertFtIn: Incorrect value passed - ${inches} inches.`);
...
function fCSCreateKL() {
  ...
  if (oldKLID) {
    ...
    const response = ui.alert("WARNING!", 
  ...
  raceClassRef.getRange(raceClassKLURL_R + 1, raceClassKLURL_C + 1).setFormula('=HYPERLINK("' + urlMyKL + '", "My KeyLine")');
  ...
  const newKLUrl = 'https://docs.google.com/spreadsheets/d/' + newKLID + '/edit';
  const htmlOutput = HtmlService.createHtmlOutput('<script>window.open("' + newKLUrl + '");google.script.host.close();</script>');
...
function fCSRefreshRuleBookURLs() {
  ...
  raceClassRef.getRange(raceClassCMandRB_R + 1, raceClassCMandEM_C + 1).setFormula('=HYPERLINK("' + urlCM + '", "Character Manual")');
  raceClassRef.getRange(raceClassCMandRB_R + 1, raceClassRBandSG_C + 1).setFormula('=HYPERLINK("' + urlRB + '", "Rule Book")');
  raceClassRef.getRange(raceClassEMandSG_R + 1, raceClassCMandEM_C + 1).setFormula('=HYPERLINK("' + urlEM + '", "Equipment Manual")');
  raceClassRef.getRange(raceClassEMandSG_R + 1, raceClassRBandSG_C + 1).setFormula('=HYPERLINK("' + urlSG + '", "Setting Guide")');
...
function fCSCalcdTiers(dieBase, numOfd) {
  ...
  if (!Number.isInteger(numOfd)) throw new Error(`In fCSdTiers, +/- # dtc has illegal # of: "${numOfd}"`);
  ...
  for (let i = 0; i < Math.abs(numOfd); i++) {
      newBase = (newBase === 1 && isPositive) ? 2 :
                (newBase === 2 && !isPositive) ? 1 :
...
function fCSPostMyRoll(rollString) {
  ...
  gSaveArraySectionToSheet(psPartyLog.ref, psPartyLog.arr, psPartyLog.url_R, psPartyLog.url_R + 6, psPartyLog_C, psPartyLog_C);
  gSaveArraySectionToSheet(gmScreen.ref, gmScreen.arr, gmScreen.url_R, gmScreen.url_R + 6, gmScreenLog_C, gmScreenLog_C);
...
function fCSLoadRollLogStats(rollLog,game,loadAllTF) {
  ...
  if (loadAllTF) {
    ...
    if (!rollLog || typeof rollLog !== 'object' || !rollLog.stats) throw new Error('Invalid rollLog object passed to fCSLoadRollLogStats.');

    // Save Roll Log to <GMScreen>
    let mySlotNum = raceClass.arr[raceClass.slot_R][raceClass.value_C].trim().slice(-1);
    if (!/^[1-9]$/.test(mySlotNum)) throw new Error(`Your <RaceClass> "GM Roll Slot" is not valid. It should look like "Slot #" where # is 1 to 9.`);
