...
function onOpen() {
  ...
  switch(spreadsheetId) {
    case g.id.mastercs: 
    case g.id.mycs: fCSCreateMenu(); break;

    case g.id.db: fDBCreateMenu(); break;

    case g.id.masterkl: 
    case g.id.mykl: fKLCreateMenu(); fKLCheckShowReminder(); break;

    case g.id.ps: fPSCreateMenu(); break;
    default:
      throw new Error(`In onOpen, I don't recognize the passed spreadsheetId of "${spreadsheetId}"`);
  }
} // End onOpen
...
function gURLFromID(id, type = 'sheet') {
  ...
  if (type.toLowerCase() !== 'sheet' && type.toLowerCase() !== 'doc') throw new Error(`In gURLFromID, the type of Google File is not understood: "${type}"`);
  
  // URL prefix based on the type
  const urlPrefix = (type.toLowerCase() === 'sheet') ? 'https://docs.google.com/spreadsheets/d/' : 'https://docs.google.com/document/d/';
...
function gCopySheet(ssFrom, sheetFrom, ssTo, sheetTo) {
  ...
  var targetSheet = gSheetRef(ssTo, sheetTo) || gSSRef(ssTo).insertSheet(sheetTo);
  ...
  if (targetMaxColumns > sourceMaxColumns) {
    targetSheet.deleteColumns(sourceMaxColumns + 1, targetMaxColumns - sourceMaxColumns);
  } else if (targetMaxColumns < sourceMaxColumns) {
    targetSheet.insertColumnsAfter(targetMaxColumns, sourceMaxColumns - targetMaxColumns);
  }

  // Copy data and formatting from gasTempSheet to targetSheet
  sourceClone.getRange(1, 1, sourceMaxRows, sourceMaxColumns).copyTo(targetSheet.getRange(1, 1), {contentsOnly: false});
...
function gSSRef(ss) {
  ...
  let ssRef;
  switch (ss) {
    case 'mycs': ssRef = SpreadsheetApp.openById(g.id.mycs); break;
    case 'mastercs': ssRef = SpreadsheetApp.openById(g.id.mastercs); break;
    case 'mykl': ssRef = SpreadsheetApp.openById(g.id.mykl); break;
    case 'masterkl': ssRef = SpreadsheetApp.openById(g.id.masterkl); break;
    case 'ps': ssRef = SpreadsheetApp.openById(g.id.ps); break;
    case 'db': ssRef = SpreadsheetApp.openById(g.id.db); break;
    default: throw new Error(`gSSRef was passed an unknown ss: "${ss}"`);
  }
  g[ss].ref = ssRef;
...
function gHeaderC(ss, sheetName, header) {
  ...
  if (headerIndex === undefined) throw new Error(`In gHeaderC, ss:"${ss}", sheetName:"${sheetName}" -- there is no header: "${header}"`);
  return headerIndex;
} // End gHeaderC
...
function gLoadTable(ss, sheetName, forceLoad = false) {
  ...
  const arr = g[ss][sheetName].arr = sheet.getRange(1, 1, maxRows, maxColumns).getValues();
...
function gTestID(ss, sheetName, testNameIDorID) {
  ...
  if (typeof testNameIDorID !== 'string' || testNameIDorID.length < 6) return false;
  ...
  if (!g[ss][sheetName].keyOf || !g[ss][sheetName].keyOf.hasOwnProperty(testID)) return false;
...
function gMakeSheet(ss, sheetName) {
  if (sheetName === null || sheetName === '') throw new Error('gGetSheetRef was passed an empty sheetName.');
  ...
  if (!ssRef.getSheetByName(sheetName)) throw new Error(`In gMakeSheet, the sheet named "${sheetName}" doesn't exist.`);
...
function gDeleteEmptyCol0DataRows(ss, sheetName) {
    ...
    for (let r = arr.length - 1; r >= firstData_R; r--) {
        if (!arr[r][0]) {  // Check if the cell in column 0 is empty
...
function gSortArraySection(arr, firstRow, lastRow, firstCol, lastCol, sortCol) {
  ...
  if (sortCol < firstCol || sortCol > lastCol) {
    throw new Error('Invalid sortCol: must be within the range of firstCol and lastCol');
  }

  // Extract the section to be sorted
  let sortSection = arr.slice(firstRow, lastRow + 1).map(row => row.slice(firstCol, lastCol + 1));
...
function gSaveArraySectionToSheet(sheetRef, entireSheetArr, top_R, bottom_R_orArrLenthMinus1, first_C, last_C) {
  ...
  const sectionToSave = entireSheetArr.slice(top_R, bottom_R + 1)
                             .map(row => row ? row.slice(first_C, last_C + 1) : Array(numCols).fill(""));
...
function gHideAll(ss) {
  ...
  sheets.forEach(sheet => {
    if (!sheet.isSheetHidden()) {
      ...
      const hideLine = hideStr.split('\n').find(line => line.startsWith('Hide:'));
      if (hideLine) {
        let cleanedHideStr = hideLine.replace('Hide:', '').replace(/\s+/g, '').replace(/^,|,$/g, ''); // Remove "Hide:", remove whitespace, remove trailing/ending ","
        ...
        hideArr.forEach(element => {
          if (/^\d+$/.test(element)) {
            ...
            const [startCol, endCol] = element.split('-').map(letter => letter.charCodeAt(0) - 64);
...
function gPutDocCache(key, value) {
  ...
  if (stringValue.length > 99000) throw new Error('In gPutDocCache, the stringified value length exceeds 99,000 characters.');
