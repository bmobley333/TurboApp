...
// Menu Functions //////////////////////////////////////////////////////////////////////////////////////////////////
// GMScreen Menu
function fDBMenuMonRefresh() {fDBRunMenuOrButton('MonRefresh');}
function fDBMenuClearRollLogs() {fDBRunMenuOrButton('ClearRollLogs');}
// Kit Menu
function fDBMenuLoadKit() {fDBRunMenuOrButton('LoadKit');}
function fDBMenuSaveKit() {fDBRunMenuOrButton('SaveKit');}
// Abilities Menu
function fDBMenuSaveVersions() {fDBRunMenuOrButton('SaveVersions');}
function fDBMenuCullBookIDs() {fDBRunMenuOrButton('CullBookIDs');}
function fDBMenuUpdateElementNotes() {fDBRunMenuOrButton('UpdateNotes');}
function fDBMenuDelBogusKitBases() {fDBRunMenuOrButton('DelBogusKitBases');}
function fDBMenuPushKitToAbil() {fDBRunMenuOrButton('PushKitToAbil');}
function fDBMenuPopulateKitFromAbil() {fDBRunMenuOrButton('PopulateKitFromAbil');}
// System Menu
function fDBMenuAuthorize() {fDBRunMenuOrButton('Authorize');}
function fDBMenuRefreshAll() {fDBRunMenuOrButton('RefreshMenu');}
// Designer Menu
function fDBMenuName_IDSync() {fDBRunMenuOrButton('MenuName_IDSync');}
function fDBMenuHideAll() {fDBRunMenuOrButton('HideAll');}
function fDBMenuUn_HideAll() {fDBRunMenuOrButton('Un_HideAll');}
// End Menu Functions


// Image Button Functions //////////////////////////////////////////////////////////////////////////////////////////////////
function fDBButtonMon() {fDBRunMenuOrButton('MonRefresh');}
function fDBButtonOpen() {fDBRunMenuOrButton('LoadKit');}
function fDBButtonSave() {fDBRunMenuOrButton('SaveKit');}
...
function fDBRunMenuOrButton(menuChoice) { 
  try {
    switch (menuChoice) {
      // GM Screen
      case 'MonRefresh': fDBMonRefresh(); break;
      case 'ClearRollLogs': fDBClearRollLogs(); break;
      // Kits
      case 'LoadKit': fDBLoadKit(); break;
      case 'SaveKit': fDBSaveKit(); break;
      // Abilities
      case 'SaveVersions': fDBSaveVersionsToBooks(); break;
      case 'CullBookIDs': fDBCullOldIDsFromBooks(); break;
      case 'UpdateNotes': fDBUpdateNotes(); break;
      case 'DelBogusKitBases': fFixBogusAbilBases(); break;
      case 'PushKitToAbil': fPushKitToAbil(); break;
      case 'PopulateKitFromAbil': fPopulateKitFromAbil(); break;
      // System Menu
      case 'Authorize': SpreadsheetApp.getUi().alert(`AUTHORIZED`, `Script Authorized!`, SpreadsheetApp.getUi().ButtonSet.OK); break;
      case 'RefreshMenu': fDBCreateMenu(); break;
      // Designer Menu
      case 'MenuName_IDSync': fDBName_IDSync(); break;
      case 'HideAll': gHideAll('db'); break;
      case 'Un_HideAll': gUn_HideAll('db'); break;
    }
  } catch (error) {
...
function fDBMonRefresh() {
  ...
  const nishColMult = (isNaN(monArr[monColMult_R][monNish_C]) || monArr[monColMult_R][monNish_C] <= 0) ? 1 : monArr[monColMult_R][monNish_C];
  const mrColMult = (isNaN(monArr[monColMult_R][monMR_C]) || monArr[monColMult_R][monMR_C] <= 0) ? 1 : monArr[monColMult_R][monMR_C];
  const vitColMult = (isNaN(monArr[monColMult_R][monVit_C]) || monArr[monColMult_R][monVit_C] <= 0) ? 1 : monArr[monColMult_R][monVit_C];
  const atkColMult = (isNaN(monArr[monColMult_R][monAtk_C]) || monArr[monColMult_R][monAtk_C] <= 0) ? 1 : monArr[monColMult_R][monAtk_C];
  const dmgColMult = (isNaN(monArr[monColMult_R][monDmg_C]) || monArr[monColMult_R][monDmg_C] <= 0) ? 1 : monArr[monColMult_R][monDmg_C];
  const defColMult = (isNaN(monArr[monColMult_R][monDef_C]) || monArr[monColMult_R][monDef_C] <= 0) ? 1 : monArr[monColMult_R][monDef_C];
  const arColMult = (isNaN(monArr[monColMult_R][monAr_C]) || monArr[monColMult_R][monAr_C] <= 0) ? 1 : monArr[monColMult_R][monAr_C];
...
if (monArr[r][monNameID_C] === '') {
  ...
  sheetRef.getRange(r + 1, monNish_C + 1, 1, monAr_C - monNish_C + 1).setFontWeight('normal');
}
 else {
      const monID = monArr[r][monID_C];
      const monIndMM = monArr[r][monIndMM_C];
      if (fDBTextisNonBold(sheetRef, monArr, r, monNish_C)) monArr[r][monNish_C] = Math.round(gGetVal('db', 'Monsters', monID, dataNish_C) * nishColMult * ((monIndMM === '') ? monMultAll : monIndMM));
      if (fDBTextisNonBold(sheetRef, monArr, r, monMR_C)) monArr[r][monMR_C] = Math.round(+gGetVal('db', 'Monsters', monID, dataMR_C) * mrColMult);
      if (fDBTextisNonBold(sheetRef, monArr, r, monVit_C)) monArr[r][monVit_C] = Math.round(gGetVal('db', 'Monsters', monID, dataVit_C) * vitColMult * ((monIndMM === '') ? monMultAll : monIndMM));
      if (fDBTextisNonBold(sheetRef, monArr, r, monAtk_C)) monArr[r][monAtk_C] = Math.round(gGetVal('db', 'Monsters', monID, dataAtk_C) * atkColMult * ((monIndMM === '') ? monMultAll : monIndMM));
      if (fDBTextisNonBold(sheetRef, monArr, r, monDmg_C)) monArr[r][monDmg_C] = Math.round(gGetVal('db', 'Monsters', monID, dataDmg_C) * dmgColMult * ((monIndMM === '') ? monMultAll : monIndMM));
      if (fDBTextisNonBold(sheetRef, monArr, r, monDef_C)) monArr[r][monDef_C] = Math.round(gGetVal('db', 'Monsters', monID, dataDef_C) * defColMult * ((monIndMM === '') ? monMultAll : monIndMM));
      if (fDBTextisNonBold(sheetRef, monArr, r, monAr_C)) monArr[r][monAr_C] = Math.round(gGetVal('db', 'Monsters', monID, dataAr_C) * arColMult *  ((monIndMM === '') ? monMultAll : monIndMM));
      monArr[r][monPic_C] = gGetVal('db', 'Monsters', monID, dataPic_C);
      monArr[r][monNotes_C] = gGetVal('db', 'Monsters', monID, dataNotes_C);
    }
  }

  gSaveArraySectionToSheet(sheetRef, monArr, monTblPremadeFirst_R, monTblPremadeLast_R, monIndMM_C, monNotes_C);
  ...
  gSaveArraySectionToSheet(sheetRef, monArr, monTblCustFirst_R, monTblCustLast_R, monActive_C, monAr_C);
} // end fDBMonRefresh
...
function fDBClearRollLogs(ss) {
  ...
  gFillArraySection(gmScreenArr,gmScreenFirst_R,gmScreenLast_R,gmScreenFirst_C,gmScreenLast_C,'');
  gSaveArraySectionToSheet(gmScreenRef,gmScreenArr,gmScreenFirst_R,gmScreenLast_R,gmScreenFirst_C,gmScreenLast_C);
...
function fDBLoadKit() {
  ...
  if (!kitID) throw new Error(`In fDBLoadKit, the selected kit "${kitNameID}" does not exist in <Kits>`);

  gFillArraySection(kitEditorArr, kitEditorDataFirst_R, kitEditorDataLast_R, kitEditorAbilityNameID_C, kitEditorMorph2_C,'');

  const kitMorphStr = gGetVal('db', 'Kits', kitID, 'MorphString');
  gSaveVal('db', 'KitEditor', kitEditorMorphStr_R, kitEditorKitMorphStr_C, kitMorphStr);
  ...
  if (kitMorphStr) {
    ...
    for (const element of morphAbilArr) {
      if (r > kitEditorDataLast_R) throw new Error(`The kit "${kitNameID}" has more abilities than will fit on the <KitEditor> table. You need to add rows to this table.`);
      const [abilID, otherMorphStr, sk1MorphStr, sk2MorphStr] = element.split('|');
      kitEditorArr[r][kitEditorAbilityNameID_C] = gGetVal('db', 'Abilities', abilID, 'Name_ID');
  ...
  gSaveArraySectionToSheet(kitEditorRef, kitEditorArr, kitEditorDataFirst_R, kitEditorDataLast_R, kitEditorAbilityNameID_C, kitEditorMorph2_C);
...
function fDBSaveKit(blankAfterSave = true) {  
  ...
  const kitID = gTestID('db', 'Kits', kitNameID);
  if (!kitID) throw new Error(`In fDBSaveKit, the selected kit "${kitNameID}" does not exist in <Kits>`);
  ...
  for (let r = kitEditor.dataFirst_R; r <= kitEditor.dataLast_R; r++) {
    ...
    if (abilID) {
      ...
      abilityMorphStr = fDBMakeAbilityMorph(otherMorphStr, morph1Str, morph2Str);
      kitMorphStr = (abilityMorphStr) ? `${kitMorphStr}\\${abilID}|${abilityMorphStr}` : kitMorphStr; // the two \\ will produce just one \
    }
  }

  // Do NOT save an empty kitMorphStr, but otherwise...
  if (kitMorphStr) gSaveVal('db', 'KitEditor', kitEditor.kitMorphString_R, kitEditor.kitMorphString_C, kitMorphStr);
  ...
  if (blankAfterSave) {
    gFillArraySection(kitEditor.arr, kitEditor.dataFirst_R, kitEditor.dataLast_R, kitEditor.nameID_C, kitEditor.morph2_C,'');
    gSaveArraySectionToSheet(kitEditor.ref, kitEditor.arr, kitEditor.dataFirst_R, kitEditor.dataLast_R, kitEditor.nameID_C, kitEditor.morph2_C);
  }
...
function fDBCleanOtherMorph_Str(morphString) {
  ...
  morphString = String(morphString).replace(/\s+/g, '')  // Remove all whitespace
  ...
  for (const element of morphArr) {
    switch (true) {
      ...
      default: throw new Error(`In fDBCleanOtherMorph_Str MorphOther has an invalid argument: "${element}"\n`);
    }
  }
...
function fDBCleanMorph1or2_Str(morphString) {
  ...
  for (const element of morphArr) {
    switch (true) {
      ...
      default:
        throw new Error(`In fDBCleanMorph1or2_Str morph has an invalid argument: "${element}"\n`);
    }
  }
...
function fDBSyncBase() {
  ...
  const kitID = gTestID('db', 'Kits', kitEd.arr[kitEd.kitNameID_R][kitEd.kitNameID_C]);
  ...
  for (let r = kitEd.dataFirst_R; r <= kitEd.dataLast_R; r++) {
    ...
    if (abilID) {
      ...
      const abilBase1isNum = (kitCantSkill) ? false : abilBase1 !== '' && !isNaN(abilBase1);
      const abilBase2isNum = (kitCantSkill) ? false : abilBase2 !== '' && !isNaN(abilBase2);
      ...
      // Else use Default Sk1 and Sk2 PLAGHE from <Abilities>
        ...
        abilSk1Morph = (kitCantSkill) ? '' : abil.arr[abil_R][abil.defaultPLAGHESk1_C];
        abilSk2Morph = (kitCantSkill) ? '' : abil.arr[abil_R][abil.defaultPLAGHESk2_C];
      }

      const finalMorph1 = fDBSetFinalMorphString(kitMorph1, abilSk1Morph, abilBase1isNum, primaryBaseTab, abilName);
      const finalMorph2 = fDBSetFinalMorphString(kitMorph2, abilSk2Morph, abilBase2isNum, primaryBaseTab, abilName);
  ...
  gSaveArraySectionToSheet(kitEd.ref, kitEd.arr, kitEd.dataFirst_R, kitEd.dataLast_R, kitEd.morph1_C, kitEd.morph2_C);
  if (kitHeader_C) gSaveArraySectionToSheet(abil.ref, abil.arr, abil.dataFirst_R, abil.dataLast_R, kitHeader_C, kitHeader_C + 1);
...
function fDBSetFinalMorphString(kitMorph, abilMorph, abilBaseIsNum, primaryTab, abilName) {
  ...
  if (!abilBaseIsNum) {
    ...
    if (kitMorph && abilMorph) {
        if (String(kitMorph) !== String(abilMorph)) {
          primaryTab[0] = primaryTab[0] || fDBPromptKitEdOrAbilAsPrimary(abilName, kitMorph, abilMorph); // fPromptKitEdOrAbilAsPrimary returns 'KitEditor' or 'Abilities'
...
function fDBSaveVersionsToBooks() {
  ...
  var sortRange = versions.ref.getRange(versions.dataFirst_R + 1, versions.first_C + 1, numRowsToSort, numColToSort);
  sortRange.sort(1);
...
function fDBNewVerBlock(versions, abilID) {
  ...
  if (firstVer_R) {
    ...
    while (lastVer_R < versions.dataLast_R && versions.arr[lastVer_R][versions.id_C] === abilID) {
      lastVer_R++;
    }
    // Decrement to stay within the last valid version
    if (lastVer_R > firstVer_R && versions.arr[lastVer_R][versions.id_C] !== abilID) {
      lastVer_R--;
    }
...
function fDBReplaceOldVerBlock(paragraphArr, start_i, newVerBlock) {
  ...
  for (; i < paragraphArr.length && keepSearching; i++) {
    ...
    } else if (paragraphArr[i].getHeading() !== DocumentApp.ParagraphHeading.NORMAL) {
...
function fDBDeleteVer2Up(docRef) {
  ...
  for (let i = paragraphs.length - 1; i >= 0; i--) {
    ...
    let match = text.match(/^v(\d+):/); // Use regex to find 'v#: ' at the start of the paragraph
    if (match && match[1] !== '1') { // Check if version number is not '1'
...
function fDBUpdateNotes() {
  ...
  gFillArraySection(elem.arr,elem.dataFirst_R,elem.dataLast_R,elem.notes_C,elem.notes_C,'')
  ...
  gSaveArraySectionToSheet(elem.ref,elem.arr,elem.dataFirst_R,elem.dataLast_R,elem.notes_C,elem.notes_C);
...
function fDBFindAllNotes(bookRef,elem) {
  ...
  for (var i = 0; i < paragraphArr.length; i++) {
      ...
      if (text.startsWith("||")) {
          ...
          for (var j = i + 1; j < paragraphArr.length && paragraphArr[j].getHeading() === DocumentApp.ParagraphHeading.NORMAL; j++) {
          ...
          barAbilArr.forEach(function(barAbil) {
              ...
              if (gTestID('db','Elements',abilID)) {  // Check if the ID is valid and found
    ...
    if (match) {
      ...
      colArr.forEach(letter => {
        ...
        for (let r = dataFirst_R; r <= dataLast_R; r++) {
          ...
          if (elementID === false) throw new Error(`In fDBName_IDSync sheet ${sheetName}'s ID not found: "${sheetArr[r][arr_C]}"`);
          sheetArr[r][arr_C] = gGetVal('db', 'elements', elementID, 'Name_ID');
        }
      // Save sheetArr to sheet from rows data_R to end of column arr_C
      gSaveArraySectionToSheet(sheet,sheetArr,dataFirst_R,dataLast_R,arr_C,arr_C);
      });
