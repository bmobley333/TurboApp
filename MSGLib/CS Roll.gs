...
function fCSMasterRoller(luckedOrFreeStr = '') {
  ...
  if (isSk1 || isSk2) {
    ...
    roll.isCombat = ['atk', 'def', 'dmg', 'ar'].includes(roll.type.toLowerCase());
  ...
  // Astral Gauntlet
  } else if (sheetName === 'Game' && activeCell.r >= game.chaos1_R && activeCell.r <= game.chaosWrist_R && activeCell.c >= game.chaosUses_C && activeCell.c <= game.chaosAbility_C) {
...
function fCSFFRoll(game, r, isSk1, luckedOrFreeStr) {
  ...
  const newObj = {
    ...
    dctTotal: 0,
    morphString: '',
    typeOriginal: isSk1 ? game.arr[r][game.sk1Typ_C] : game.arr[r][game.sk2Typ_C],
...
function fCSCalcMorphEffects(game, roll, r) {
  const crudeMorphString = (roll.isSk1) ? `${game.arr[r][game.permMorph1_C]},${game.arr[r][game.morph1_C]}` : `${game.arr[r][game.permMorph2_C]},${game.arr[r][game.morph2_C]}`;

  const cleanMorphString = String(crudeMorphString).replace(/\s+/g, '')     // Remove all whitespace
  ...
  morphArr.forEach(element => {
    switch (true) {
      ...
      case ['~', 'R', 'O', 'Y', 'G', 'B'].includes(element): {
        ...
        const maxIndex = Math.max(colors.indexOf(roll.focusColor), colors.indexOf(element));
      ...
      // Calc roll types
      case element === 'SKI':
      case element === 'SK': roll.type = 'Sk'; break;
      case element === 'UN':
      case element === 'UNSK': roll.unSkilled = true; break;
      case element === 'ATK': roll.type = 'Atk'; break;
      case element === 'DMG': roll.type = 'DMG'; break;
      case element === 'DEF': roll.type = 'Def'; break;
      case element === 'ARM':
      case element === 'AR': roll.type = 'AR'; break;
      case element === 'FREE': roll.isFreeRoll = true; break;
      case element === 'LUCK': roll.isLuckedRoll = true; break;
    
      // Calc two letter + Positive Number
      case element.startsWith('**'): roll.postMult *= Number(element.substring(2)); break;
      case element.startsWith('//'): roll.postMult /= Number(element.substring(2)); break; 
      case element.startsWith('=='): roll.postEquals = Math.max(roll.postEquals, Math.abs(Number(element.substring(2)))); break;
      case element.startsWith('++'): roll.postPlus += Number(element.substring(2)); break;
      case element.startsWith('--'): roll.postPlus -= Number(element.substring(2)); break;
      ...
      case /^#\d+(\.\d+)?$/.test(element.toString()): {
        const truncatedValue = Math.trunc(Number(element.toString().replace(/^#/, '')));
        roll.numOfRolls = Math.min(50, Math.max(roll.numOfRolls, truncatedValue)); 
        break;
      }
      ...
      case element.startsWith('='): roll.equals = Math.max(roll.equals, Math.abs(Number(element.substring(1)))); break;
      case element.startsWith('+'): roll.plus += Number(element.substring(1)); break;
      case element.startsWith('-'): roll.plus -= Number(element.substring(1)); break;
      case element.startsWith('^'): roll.difficulty = Math.max(roll.difficulty, Number(element.substring(1))); break;  
      case element.startsWith('*'): roll.mult *= Number(element.substring(1)); break;
      case element.startsWith('/'): roll.mult /= Number(element.substring(1)); break;

      default: throw new Error(`I don't understand this morph: "${element}"`);
    }
  });
...
function fCSdTiers(roll) {
  ...
  if (!Number.isInteger(roll.dctTotal)) throw new Error(`In fCSdTiers, +/- # dct has illegal # of: "${roll.dctTotal}"`);
...
function fCSCalcFocus(roll) {
  ...
  if (!metaColorArr.includes(focusColorUpper)) throw new Error(`In fCSCalcFocus, there is an illegal focus color of "${roll.focusColor}"`);
...
function fCSSpendLuckBox(game) {
  ...
  if (!Number.isInteger(luckNum) || luckNum <= 0) throw new Error(`At "${luckNum}" Luck Boxes, you are out of Luck.`);
...
function fCSSpendMetaColor(game, roll, metaColor) {
  ...
  if (!metaColorArr.includes(metaColor.toUpperCase())) {
    SpreadsheetApp.getUi().alert(`In function fCSSpendMetaColor a non meta color of "${metaColor}" was passed.`);
    return;
  }

  let metaWasSpent = false;
  let metaSpend_C = game.metaR_C + metaColorArr.indexOf(metaColor.toUpperCase());
  ...
  while (metaSpend_C <= game.metaB_C) {
    if (game.arr[game.metaChnl_R][metaSpend_C] > 0) {
      ...
      game.arr[game.metaChnl_R][metaSpend_C] = newMetaNum === 0 ? '' : newMetaNum;
      metaWasSpent = true;
      break;
    }
  ...
  if (metaWasSpent) {
    ...
    const response = ui.alert(`You do not have the needed @${metaColor}.\n\nContinue Anyway?`, ui.ButtonSet.YES_NO);
    if (response !== ui.Button.YES) throw new Error('Roll canceled due to lack of Meta.');
...
function fCSSpendAction(game, roll, r) {
  ...
  if (actWasSpent) {
    ...
    const ui = SpreadsheetApp.getUi();
    const response = ui.alert(`You do not have the needed Act(${actCost}).<br>\nContinue Anyway?`, ui.ButtonSet.YES_NO);
    if (response !== ui.Button.YES) throw new Error('Roll canceled due to lack of Action.');
...
function fCSSpendUses(game, roll, r) {
  ...
  if (currentUses > 0) {
    ...
    const ui = SpreadsheetApp.getUi();
    const response = ui.alert(`Your Uses is at 0.\n\nContinue Anyway?`, ui.ButtonSet.YES_NO);
    if (response !== ui.Button.YES) throw new Error('Roll canceled due to lack of Uses.');
...
function fCSSelectAbilRollFunc(game, roll, r) {
  ...
  roll.base = (!roll.base && roll.equals) ? Math.round(Math.max(1, roll.equals)) : roll.base;

  if (roll.base) {
    roll.morphedBase = (roll.equals) ? fCSCombineBaseAndArr(roll.equals,roll.combineArr) : fCSCombineBaseAndArr(roll.base,roll.combineArr);
    roll.morphedBase = Math.round(Math.max(1, roll.morphedBase * roll.mult + roll.plus)); 
    ...
    if (roll.isCombat) {
      if (monRoster && monRoster.hasMonsters) {
        ...
        throw new Error(`Please select a Monster Table opponent for your ${roll.name} ${roll.type} roll.`);
      }
    } else {
      fCSCalcNonCombatRoll(roll);
      if (roll.id === 'k97cmz' && r === game.nishAtr_R) fCSDoNishExtras(game,roll);
    }
  } else {
...
function fCSFFGetMonsters(game, roll) {
  ...
  for (let r = game.custMon_R; r <= game.monsterLast_R; r++) {
    ...
    if (numOfMon) {
      ...
      newMonStats.name = (r === 1) ? `Custom: ${game.arr[r][game.monsterName_C]}` : game.arr[r][game.monsterName_C] || 'NoName';
      newMonStats.atk = isNaN(Number(game.arr[r][game.monsterAtk_C])) ? 0 : Math.max(0, Number(game.arr[r][game.monsterAtk_C]));
      newMonStats.dmg = isNaN(Number(game.arr[r][game.monsterDmg_C])) ? 0 : Math.max(0, Number(game.arr[r][game.monsterDmg_C]));
      newMonStats.def = isNaN(Number(game.arr[r][game.monsterDef_C])) ? 0 : Math.max(0, Number(game.arr[r][game.monsterDef_C]));
      newMonStats.ar = isNaN(Number(game.arr[r][game.monsterAR_C])) ? 1 : Math.max(1, Number(game.arr[r][game.monsterAR_C]));
...
function fCSSortMonsters(roll, monRoster) {
  ...
  switch (pcSkType) {
    case 'ATK': sortKey = 'def'; break;
    case 'DMG': sortKey = 'ar'; break;
    case 'DEF': sortKey = 'atk'; break;
    case 'AR': sortKey = 'dmg'; break;
    default: throw new Error(`In fCSSortMonsters, unknown skill type: ${pcSkType}`);
  }
...
function fCSCalcCombatVsMonRoll(game, roll, monRoster) {
  ...
  for (const monster of monRoster.list) {
    ...
    for (let i = 1; i <= monster.numOf; i++) {
      ...
      // Monster Name
      thisRollLog += (i === 1 && monster.name) ? `|<br>--- ${monster.name} ---<br>` : '';
      
      const rollCounter = (monster.numOf === 1) ? '' : `${i}&gt `;

      // if isFirstMon apply Meta Focus to tempMophedBase
      const tempMorphedBase = (isFirstMon) ? roll.morphedBase + roll.focusPlus : roll.morphedBase; 
      rollBaseStr = (roll.base === tempMorphedBase) ? `(${roll.base})` : `(${roll.base}-&gt${tempMorphedBase})`;

       // Based on roll.type and monster stats calculate: rollResultStr, difficultyStr
      const [rollResultStr, difficultyStr] = fCSCalcCombatVsMonResultStringAndDiffStrings(roll, monster);

      roll.finalTC = (isFirstMon) ? fCStc() : '';

      thisRollLog += `${rollCounter}${roll.unSkilled ? 'UnSk' : ''}${roll.type}${rollBaseStr} ${rollResultStr}${difficultyStr} ${roll.finalTC}<br>`;
    }
  ...
  if (roll.totalPCWnds) {
    ...
    game.arr[game.vitTbl_R][game.vit1st_C] = parseFloat(game.arr[game.vitTbl_R][game.vit1st_C]) || 0;
    game.arr[game.vitTbl_R][game.vit1st_C] += roll.totalPCWnds;
    game.arr[game.vitTbl_R][game.vitNow_C] = parseFloat(game.arr[game.vitTbl_R][game.vitNow_C]) || 0;
    game.arr[game.vitTbl_R][game.vitNow_C] -= roll.totalPCWnds;
    thisRollLog += `Vit Now = ${game.arr[game.vitTbl_R][game.vitNow_C]}/${game.arr[game.vitTbl_R][game.vitMax_C]}<br>`;
    if (game.arr[game.vitTbl_R][game.vitNow_C] < 0) thisRollLog += `${g.redBS}WARNING: CRITICAL DMG${g.endS}<br>`;
  }

  const freeOrLucked = (roll.isFreeRoll || roll.isLuckedRoll) ? (roll.isLuckedRoll ? `${g.redBS}LUCKED ROLL${g.endS}<br>` : `${g.redBS}FREE ROLL${g.endS}<br>`) : '';
  const rollTypeString = (roll.typeOriginal === roll.type) ? roll.type : `${roll.typeOriginal} as ${roll.type}`;
...
function fCSCalcCombatVsMonResultStringAndDiffStrings(roll, monster) {
  ...
  switch (roll.type.toUpperCase()) {
    case 'ATK':
      const pcAtk = roll.result = (roll.postEquals) ? Math.round(Math.max(1, roll.postEquals)) : (roll.unSkilled) ? fCSRollUnSkilled(roll.base) : fCSDSk(roll.base);
      const pcPostAtk = roll.morphedResult = pcAtk * roll.postMult + roll.postPlus;
      rollResultStr = (pcAtk === pcPostAtk) ? `~${pcAtk}` : `~${pcAtk}-&gt${pcPostAtk}`;

      // Get the correct monster stat
      const monDef = roll.difficulty = monster.def;
      const pcAtkResult = pcPostAtk - monDef;
      difficultyStr = (pcAtkResult >= 0) ? `^${monDef} = ${g.grnBS}+${pcAtkResult}${g.endS}` : `^${monDef} = ${g.redItS}${pcAtkResult}${g.endS}`;  // Adds the '+' for positive results
      break;

    case 'DMG':
      // roll custom fCSDDmg
      const pcDmg = roll.result = (roll.postEquals) ? Math.round(Math.max(1, roll.postEquals)) : (roll.unSkilled) ? fCSRollUnSkilledDmg(roll.base) : fCSDDmg(roll.base);

      // Don't apply post roll adjustments (roll.postMult + roll.postPlus) until after /AR so... there is no roll.morphedResult
      rollResultStr = `~${pcDmg}`;

      // Get the correct monster stat
      const monArmor = (!monster.ar) ? 1 : Math.max(1, Math.round((monster.ar / 10) * 10) / 10);
      const monWounds = Math.round(pcDmg / monArmor);
      const monPostWounds = (monWounds) ? monWounds * roll.postMult + roll.postPlus : 0;
      const monWndSlideStr = (monWounds === monPostWounds) 
        ? (monPostWounds >= 0 ? `${g.grnBS}+${monPostWounds}${g.endS}` : `${g.redItS}${monPostWounds}${g.endS}`) 
        : (monPostWounds >= 0 ? `${monWounds}-&gt+${g.grnBS}${monPostWounds}${g.endS}` : `${monWounds}-&gt${g.redItS}${monPostWounds}${g.endS}`);
      roll.monWndsAccumultator += monPostWounds;
      difficultyStr = `/${monArmor} = ${monWndSlideStr}`;
      break;

    case 'DEF':
      const pcDef = roll.result = (roll.postEquals) ? Math.round(Math.max(1, roll.postEquals)) : (roll.unSkilled) ? fCSRollUnSkilled(roll.base) : fCSDef(roll.base);
      const pcPostDef = roll.morphedResult = pcDef * roll.postMult + roll.postPlus;
      rollResultStr = (pcDef === pcPostDef) ? `~${pcDef}` : `~${pcDef}-&gt${pcPostDef}`;

      // Get the correct monster stat
      const monAtk = roll.difficulty = monster.atk;
      const pcDefResult = pcPostDef - monAtk;
      difficultyStr = (pcDefResult >= 0) ? `^${monAtk} = ${g.grnBS}+${pcDefResult}${g.endS}` : `^${monAtk} = ${g.redItS}${pcDefResult}${g.endS}`;  // Adds the '+' for positive results
      break;
    ...
    case 'AR':
      ...
      const pcAR = roll.result = (roll.postEquals) ? Math.round(Math.max(1, roll.postEquals)) : (roll.unSkilled) ? fCSRollUnSkilledArmor(roll.base) : fCSDArmor(roll.base);
      const pcPostAR = Math.max(1,pcAR * roll.postMult + roll.postPlus);
      const pcARSlideStr = (pcAR === pcPostAR) ? `~${pcPostAR}` : `~(${pcAR}-&gt${pcPostAR})`;
      const pcWnds = Math.round(monDmg / pcPostAR);
      roll.pcWndsAccumulator += (pcWnds >= 0) ? pcWnds : 0;
      difficultyStr = (pcWnds > 0) ? `/${pcARSlideStr} = ${g.redItS}-${pcWnds}${g.endS}` : `/${pcARSlideStr} = ${g.grnBS}0${g.endS}`;
      break;
  }
...
function fCSNishCritTrem(nishtc) {
  ...
  const ct_C = (isCrit) ? gHeaderC('db','Crit','NishCrit') : gHeaderC('db','Crit','NishTrem');
  ...
  let nishCTTxt = (isCrit) ? `${g.redBS}CRITICAL NISH${g.endS}<br>` : `${g.treS}TREMENDOUS NISH${g.endS}<br>`
...
function fCalcNishOnEffects(game) {
  ...
  for (let r = game.abilTblStart_R; r <= game.gearTblEnd_R; r++) {
    ...
    let abilOn = (typeof game.arr[r][game.on_C] === 'string') ? game.arr[r][game.on_C].toUpperCase() : game.arr[r][game.on_C];

    switch (true) {
      case abilOn === 'E': abilOn = 'E'; break;
      case abilOn === '*': abilOn = '*'; break;
      case !isNaN(abilOn) && abilOn >= '2': abilOn = (Number(abilOn) - 1).toString(); break;
...
function fCSCalcNonCombatRoll(roll) {
  ...
  for (let i = 1; i <= roll.numOfRolls; i++) {
    ...
    const tempMorphedBase = (i === 1) ? roll.morphedBase + roll.focusPlus : roll.morphedBase; 
    const rollBaseStr = (roll.base === tempMorphedBase) ? `(${roll.base})` : `(${roll.base}-&gt${tempMorphedBase})`;

    // roll.result is either roll.postEquals or UnSkilled (fCSRollUnSkilled) or regular skill (fCSDSk) 
    roll.result = (roll.postEquals) ? Math.round(Math.max(1,roll.postEquals)) : ((roll.unSkilled) ? fCSRollUnSkilled(roll.base) : fCSDSk(roll.base));
    roll.morphedResult = roll.result * roll.postMult + roll.postPlus;

    const rollResultStr = (roll.result === roll.morphedResult) ? `~${roll.result}` : `~${roll.result}-&gt${roll.morphedResult}`;
    let difficultyStr = '';
    if (roll.difficulty) {
      let difResult = roll.morphedResult - roll.difficulty;
      difficultyStr = (difResult >=0) ? `^${roll.difficulty} =  ${g.grnBS}+${difResult}${g.endS}` : `^${roll.difficulty} = ${g.redItS}${difResult}${g.endS}`;  // Adds the '+' for positive results
    }
...
function fCSPerformUseChaosGem(game,roll, r) {
  ...
  if (!Number.isInteger(chaosUses) || chaosUses <= 0) throw new Error(`With "${chaosUses} uses," you can't use "${chaosAbilDesc}"`);
  chaosUses --;
...
function fCSSaveAbilRollEffects(game, roll) {
  ...
  gSaveArraySectionToSheet(game.ref, game.arr, game.meta_R, game.dataLast_R, game.rollLog_C, game.monsterSize_C);
...
function fCStc() { 
  const roll = fCSd(16);
  return (roll === 1) ? `${g.treS}T${g.endS}` : (roll === 2) ? `${g.redBS}C${g.endS}` : ''; 
} // End fCStc
